# core/syslog_parser.py
import sys, os
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from Projects.soc.sentinellite.utils.config_loader import load_config
import json, os, re
from datetime import datetime, timezone
now = datetime.now(timezone.utc)

IOC_PATH = "feeds/output/consolidated_iocs.json"
ALERT_OUTPUT_PATH = "output/alerts_syslog.json"
...
config = load_config()
SYSLOG_PATH = config.get("logs", {}).get("syslog", "/var/log/syslog")

def main():
    print("[*] Loading IOCs...")
    iocs = load_iocs()
    indicators = extract_indicators(iocs)
    print(f"[*] Loaded {len(indicators)} indicators (IPs/domains)")
    print(f"[*] Scanning {SYSLOG_PATH} for matches...")
    matches = _scan_file_for_indicators(SYSLOG_PATH, indicators, "syslog")
